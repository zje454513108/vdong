...
class FavoriteController extends CommonController {
    ...
    public function favoriteList(){
        ...
        foreach ($list['result'] as $k => $v) {
            $list['result'][$k]['thumb'] = C('IMAGE_RESOURCE') . '/' . $v['thumb'];
        }
    ...
    public function favoriteDel(){
        ...
        $data['num'] = $result;
        if($result){
            $this->outFormat($data,L('SUCCESS'),L('CODE_ONE'));
        }else{
            $this->outFormat($data,L('F_DELETED'),L('F_CODE_ZERO'));
        }
    }
    /**
     * 收藏设置 
    ...
    public function favoriteSet(){
        ...
        if (empty($data)) {
            ...
            $ajaxdata['num'] = $result;
            if($result){
                $this->outFormat($ajaxdata,L('F_SUCCESS'),L('F_CODE_TWO'));
            }
        } else {
            if (empty($data['deleted'])) {
                ...
                $ajaxdata['num'] = $result;
                $this->outFormat($ajaxdata,L('F_CANCEL'),L('F_CODE_THREE'));
            } else {
                // 设置收藏
