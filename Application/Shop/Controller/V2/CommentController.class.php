...
class CommentController extends CommonController {
    ...
    public function commentSave(){
        ...
        if (empty($order)) {
            $ajaxdata['num'] = 0;
            $this->outFormat($ajaxdata,L('ORDER_ERROR',array('orderid' => $orderid)),L('C_CODE_ZERO'));
        }
        if ($order['status'] != 3 && $order['status'] != 4) {
            $ajaxdata['num'] = 0;
            $this->outFormat($ajaxdata,L('ORDER_NON_RECEIPT',array('orderid' => $orderid)),L('C_CODE_ONE'));
        }
        if ($order['iscomment'] >= 2) {
            $ajaxdata['num'] = 0;
            $this->outFormat($ajaxdata,L('ISCOMMENT'),L('C_CODE_TWO'));
        }
        // 获取用户信息
        ...
        foreach ($comments as $c) {
            ...
            if (empty($old_c)) {
                ...
                $ajaxdata['num'] = $result;
                if($result){
                    $this->outFormat($ajaxdata,L('SUCCESS'),L('CODE_ONE'));
                }
            } else {
