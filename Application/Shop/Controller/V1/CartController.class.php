...
class CartController extends CommonController {
    ...
    public function index() {
        ...
        if ($list) {
            ...
            $this->outFormats('null', '本页没有数据了！', $p, $totalPage, 0);
        }
    }
    ...
    public function update() {
        ...
        if ($info == false) {
            $this->outFormat('null', '购物车数据未找到！', 8004);
        }
...
//        查询属性
        ...
        if (empty($option)) {
            ...
            if (empty($optionid)) {
                $this->outFormat('null', '属性不存在！', 8008);
            }
            $has_op = false;
            foreach ($option as $v) {
                if ($v['id'] == $optionid) {
        ...
        if ($has_op == false) {
            $this->outFormat('null', '属性不存在！', 8009);
        }
//        查询购物车是否存在
        $where = array(
            'uniacid' => $this->token,
