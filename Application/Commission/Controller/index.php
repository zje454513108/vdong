...
if ($_W['isajax']) {
    ...
    $member['customercount']    = pdo_fetchcolumn('select count(id) from ' . tablename('ewei_shop_member') . ' where agentid=:agentid and ((isagent=1 and status=0) or isagent=0) and uniacid=:uniacid limit 1', array(
        ':uniacid' => $_W['uniacid'],
        ':agentid' => $member['id']
    ));
