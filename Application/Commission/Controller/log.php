...
if ($_W['isajax']) {
    if ($operation == 'display') {
        ...
        $id    = intval($_GPC['id']);
        $apply = pdo_fetch('select * from ' . tablename('ewei_shop_commission_apply') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(
            ':id' => $id,
            ':mid' => $mid,
            ':uniacid' => $uniacid
        ));
        ...
        $pindex = max(1, intval($_GPC['page']));
        $psize  = 5;
        $apply  = pdo_fetch('select orderids from ' . tablename('ewei_shop_commission_apply') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(
            ':id' => $id,
            ':mid' => $mid,
            ':uniacid' => $uniacid
        ));
